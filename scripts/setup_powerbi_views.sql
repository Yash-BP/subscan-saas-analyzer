-- ====================================================================
-- SAAS WASTE ANALYTICS: POWER BI VIEWS
-- Target DB: company_database.db (SQLite)
-- ====================================================================

DROP VIEW IF EXISTS vw_executive_waste_summary;
DROP VIEW IF EXISTS vw_wasted_licenses_detail;

-- 2. Create the Detail View (Fixed to include all 3 waste types)
CREATE VIEW vw_wasted_licenses_detail AS
SELECT 
    e.emp_id,
    e.name AS employee_name,
    e.department,
    e.status AS employee_status,
    s.tool_name,
    s.monthly_cost,
    u.last_login_date,
    CAST(u.logins_last_30_days AS INTEGER) AS logins_last_30_days,
    -- Simple logic to categorize the waste
    CASE 
        WHEN LOWER(TRIM(e.status)) = 'terminated' THEN 'Terminated Employee'
        WHEN CAST(u.logins_last_30_days AS INTEGER) <= 0 THEN 'Zombie License'
        WHEN CAST(u.logins_last_30_days AS INTEGER) <= 4 THEN 'At-Risk License'
    END AS waste_category
FROM 
    employees e
JOIN 
    subscriptions s ON e.emp_id = s.emp_id
JOIN 
    usage_logs u ON s.sub_id = u.sub_id
WHERE 
    LOWER(TRIM(s.license_status)) = 'active'
    AND (
        LOWER(TRIM(e.status)) = 'terminated' 
        OR CAST(u.logins_last_30_days AS INTEGER) <= 4
    );

-- 3. Create the Executive Summary View
CREATE VIEW vw_executive_waste_summary AS
SELECT 
    tool_name,
    waste_category,
    COUNT(emp_id) AS unused_license_count,
    SUM(monthly_cost) AS total_monthly_waste,
    (SUM(monthly_cost) * 12) AS total_annualized_waste
FROM 
    vw_wasted_licenses_detail
GROUP BY 
    tool_name, waste_category
ORDER BY 
    total_monthly_waste DESC;